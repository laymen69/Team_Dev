...
const menuItems = [
  { icon: 'bell', label: 'Notifications', badge: 3, path: '/admin/notifications' },
    ...
    <Card style={styles.profileCard}>
      <View style={styles.profileHeaderRow}>
        ...
        <View style={styles.profileInfo}>
          ...
          <Text style={[styles.profileZone, { color: colors.textSecondary }]}>Headquarters - Administration</Text>
        </View>
      </View>
      <View style={[styles.separator, { backgroundColor: colors.border }]} />
      <View style={styles.contactRow}>
        <Feather name="mail" size={14} color={colors.textSecondary} />
        <Text style={[styles.contactText, { color: colors.textSecondary }]}>{user?.email || 'admin@fieldforce.com'}</Text>
      </View>
    </Card>
  );
};
  ...
  const handleUpdateProfile = async (data: any) => {
    ...
    try {
      ...
      if (updated) {
        ...
        updateUser(updated); // Sync global auth context
...
const styles = StyleSheet.create({
  ...
  statValue: { fontSize: 20, fontFamily: Fonts.headingXBold },
