...
export default function BeforeAfterEvent() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const result = await ReportService.create({
                name: formData.category || 'Before/After Report',
                notes: formData.description,
                type: 'Before/After',
                status: 'pending',
                before_image: beforePhoto || undefined,
                after_image: afterPhoto || undefined,
                visits_planned: 0,
                visits_completed: 0,
            });
    ...
    const renderStep1 = () => (
        <Card style={styles.formCard}>
            <Text style={[styles.stepTitle, { color: colors.text }]}>Step 1: Before Photo</Text>
            <Text style={[styles.stepDesc, { color: colors.textSecondary }]}>Capture the initial state of the shelf or area.</Text>
            ...
            <View style={styles.inputGap}>
                {beforePhoto ? (
                    ...
                    <View style={styles.photoActions}>
                        <Card onPress={() => pickImage('before', 'camera')} style={styles.photoCard} elevation="md">
                            <Ionicons name="camera" size={32} color={colors.primary} />
                            <Text style={[styles.photoLabel, { color: colors.text }]}>Take Photo</Text>
                        </Card>
                        <Card onPress={() => pickImage('before', 'library')} style={styles.photoCard} elevation="md">
    ...
    const renderStep2 = () => (
        <Card style={styles.formCard}>
            <Text style={[styles.stepTitle, { color: colors.text }]}>Step 2: After Photo</Text>
            <Text style={[styles.stepDesc, { color: colors.textSecondary }]}>Capture the final result after your adjustments.</Text>
            ...
            <View style={styles.inputGap}>
                {afterPhoto ? (
                    ...
                    <View style={styles.photoActions}>
                        <Card onPress={() => pickImage('after', 'camera')} style={styles.photoCard} elevation="md">
                            <Ionicons name="camera" size={32} color={colors.primary} />
                            <Text style={[styles.photoLabel, { color: colors.text }]}>Take Photo</Text>
                        </Card>
                        <Card onPress={() => pickImage('after', 'library')} style={styles.photoCard} elevation="md">
                            <Ionicons name="images" size={32} color={colors.primary} />
                            <Text style={[styles.photoLabel, { color: colors.text }]}>Gallery</Text>
                        </Card>
                    </View>
    ...
    const renderStep3 = () => (
        <Card style={styles.formCard}>
            <Text style={[styles.stepTitle, { color: colors.text }]}>Step 3: Details</Text>
            <Text style={[styles.stepDesc, { color: colors.textSecondary }]}>Provide additional context for this report.</Text>
